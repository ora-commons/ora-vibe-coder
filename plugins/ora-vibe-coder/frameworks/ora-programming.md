...
- Do not restate or approximate the Programming Loop's instructions.
...
- The Programming Loop has one universal framework and a small adapter for each supported initiating host: Codex, Claude Code, ZCode, Hermes, Qwen Code, and MiniMax Code.
- Select the adapter that matches the recipient which will create and coordinate executor and reviewer sessions; include exactly that adapter in the handoff.
- Do not send an adapter collection or let the recipient guess.
- A model, provider, or service used for an optional independent opinion is a review target, not the initiating host. For example, a Codex recipient remains on the Codex adapter even if an authorized review is sent to a Claude model.
- Bridge, when applicable, is only the courier for that review; it does not replace the Loop coordinator or select a host adapter.
- Confirm native mechanics from the installed companion and host, not from a product name alone.
- The adapter must establish: a fresh executor; a different fresh reviewer; complete result retrieval; and bounded waiting.
- An installation or package check can prove resources and wiring; it cannot prove that a live account, model, provider, permission mode, or operating-system combination has been qualified. Carry any such limit into the assignment and final handback.
- Never install a coding tool, authenticate, discover credentials, choose a paid route, or switch providers merely to make an adapter usable.
- If the selected recipient cannot establish its native route: show the complete manual delivery option and disclose that execution or review isolation remains unestablished until a qualified recipient returns evidence.
- If it cannot establish the required automatic Programming transfer: report that concrete limitation instead of asking the user to manage worker conversations.
...
- After the Loop reaches its approved finish line, require its handback to identify the inspectable candidate, documentation, check evidence, delivery and cleanup state, and remaining work.
- A staffed coordinator reads that evidence and delegates any substantive inspection.
- Present the result and, when authorized, update only the overview's `Programming` field under the shared rules.
- In standalone use: recommend [Ora Verification](ora-verification.md) and stop; do not invoke it secretly.
- In guided use: return to the owner for the next authorized assignment.
- If only a handoff was prepared, disclose that execution is unestablished and leave result status unchanged.
- A Loop handback does not complete a larger project with remaining verification or delivery.
